...
def get_asset_package_prefix(asset_type:AssetType):
    return f"oss_asset_{asset_type.name.lower()}_"
...
class PackageManager(object):
    ...
    def get_module(cls,type:AssetType,module_name=None):
        if type != AssetType.NONE:
            if module_name is None:
                return find_module("oss_resource_lib.types",type.name.lower())
            else :
                return find_module(f"oss_resource_lib.types.{type.name.lower()}",module_name)
        else:
