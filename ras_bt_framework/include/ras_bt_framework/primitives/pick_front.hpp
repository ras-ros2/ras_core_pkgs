...
namespace ras_bt_framework
{
    ...
    class PickFront : public BT::SyncActionNode
    {
    public:
        PickFront(const std::string& name, const BT::NodeConfig& config = {}, rclcpp::Node::SharedPtr node=nullptr) :
            BT::SyncActionNode(name, config), node_(node) 
        {
            ...
            this->setPostTickFunction(std::bind(&PickFront::postTick, this, std::placeholders::_1, std::placeholders::_2));
            initialize();
        }
        ...
        void initialize() {
            RCLCPP_INFO(node_->get_logger(), "PickFront initialized");
            pick_front_client = node_->create_client<ras_interfaces::srv::PickFront>("/pick_front");
        }
        ...
        BT::NodeStatus tick() override {
            std::cout << (this->name()) << " executing PickFront" << std::endl;
            ...
            // 3. Send pick front request
            auto request = std::make_shared<ras_interfaces::srv::PickFront::Request>();
            request->target_pose = pose_msg.value();
            request->grip_state = grip_state;

            auto future = pick_front_client->async_send_request(request);
            if (rclcpp::spin_until_future_complete(node_, future) != 
                rclcpp::FutureReturnCode::SUCCESS)
            {
                RCLCPP_ERROR(node_->get_logger(), "Failed to send pick front request");
                return BT::NodeStatus::FAILURE;
            }

            auto response = future.get();
            if (!response->success) {
                RCLCPP_ERROR(node_->get_logger(), "Pick front operation failed: %s", response->message.c_str());
                return BT::NodeStatus::FAILURE;
            }

            RCLCPP_INFO(node_->get_logger(), "PickFront completed successfully");
            return BT::NodeStatus::SUCCESS;
        }

    private:
        rclcpp::Node::SharedPtr node_;
        rclcpp::Client<ras_interfaces::srv::PickFront>::SharedPtr pick_front_client;
    };
} 