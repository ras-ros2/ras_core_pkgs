...
namespace ras_bt_framework
{
    ...
    class PickLeft : public BT::SyncActionNode
    {
    public:
        PickLeft(const std::string& name, const BT::NodeConfig& config = {}, rclcpp::Node::SharedPtr node=nullptr) :
            BT::SyncActionNode(name, config), node_(node) 
        {
            ...
            this->setPostTickFunction(std::bind(&PickLeft::postTick, this, std::placeholders::_1, std::placeholders::_2));
            initialize();
        }
        ...
        void initialize() {
            RCLCPP_INFO(node_->get_logger(), "PickLeft initialized");
            pick_left_client = node_->create_client<ras_interfaces::srv::PickLeft>("/pick_left");
        }
        ...
        BT::NodeStatus tick() override {
            std::cout << (this->name()) << " executing PickLeft" << std::endl;
            ...
            // 3. Send pick left request
            auto request = std::make_shared<ras_interfaces::srv::PickLeft::Request>();
            request->target_pose = pose_msg.value();
            request->grip_state = grip_state;

            auto future = pick_left_client->async_send_request(request);
            if (rclcpp::spin_until_future_complete(node_, future) != 
                rclcpp::FutureReturnCode::SUCCESS)
            {
                RCLCPP_ERROR(node_->get_logger(), "Failed to send pick left request");
                return BT::NodeStatus::FAILURE;
            }

            auto response = future.get();
            if (!response->success) {
                RCLCPP_ERROR(node_->get_logger(), "Pick left operation failed: %s", response->message.c_str());
                return BT::NodeStatus::FAILURE;
            }

            RCLCPP_INFO(node_->get_logger(), "PickLeft completed successfully");
            return BT::NodeStatus::SUCCESS;
        }

    private:
        rclcpp::Node::SharedPtr node_;
        rclcpp::Client<ras_interfaces::srv::PickLeft>::SharedPtr pick_left_client;
    };
} 