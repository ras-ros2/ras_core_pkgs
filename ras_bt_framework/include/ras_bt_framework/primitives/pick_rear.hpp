...
namespace ras_bt_framework
{
    ...
    class PickRear : public BT::SyncActionNode
    {
    public:
        PickRear(const std::string& name, const BT::NodeConfig& config = {}, rclcpp::Node::SharedPtr node=nullptr) :
            BT::SyncActionNode(name, config), node_(node) 
        {
            ...
            this->setPostTickFunction(std::bind(&PickRear::postTick, this, std::placeholders::_1, std::placeholders::_2));
            initialize();
        }
        ...
        void initialize() {
            RCLCPP_INFO(node_->get_logger(), "PickRear initialized");
            pick_rear_client = node_->create_client<ras_interfaces::srv::PickRear>("/pick_rear");
        }
        ...
        BT::NodeStatus tick() override {
            std::cout << (this->name()) << " executing PickRear" << std::endl;
            ...
            // 3. Send pick rear request
            auto request = std::make_shared<ras_interfaces::srv::PickRear::Request>();
            request->target_pose = pose_msg.value();
            request->grip_state = grip_state;

            auto future = pick_rear_client->async_send_request(request);
            if (rclcpp::spin_until_future_complete(node_, future) != 
                rclcpp::FutureReturnCode::SUCCESS)
            {
                RCLCPP_ERROR(node_->get_logger(), "Failed to send pick rear request");
                return BT::NodeStatus::FAILURE;
            }

            auto response = future.get();
            if (!response->success) {
                RCLCPP_ERROR(node_->get_logger(), "Pick rear operation failed: %s", response->message.c_str());
                return BT::NodeStatus::FAILURE;
            }

            RCLCPP_INFO(node_->get_logger(), "PickRear completed successfully");
            return BT::NodeStatus::SUCCESS;
        }

    private:
        rclcpp::Node::SharedPtr node_;
        rclcpp::Client<ras_interfaces::srv::PickRear>::SharedPtr pick_rear_client;
    };
} 