...
namespace ras_bt_framework
{
    ...
    class PickRight : public BT::SyncActionNode
    {
    public:
        PickRight(const std::string& name, const BT::NodeConfig& config = {}, rclcpp::Node::SharedPtr node=nullptr) :
            BT::SyncActionNode(name, config), node_(node) 
        {
            ...
            this->setPostTickFunction(std::bind(&PickRight::postTick, this, std::placeholders::_1, std::placeholders::_2));
            initialize();
        }
        ...
        void initialize() {
            RCLCPP_INFO(node_->get_logger(), "PickRight initialized");
            pick_right_client = node_->create_client<ras_interfaces::srv::PickRight>("/pick_right");
        }
        ...
        BT::NodeStatus tick() override {
            std::cout << (this->name()) << " executing PickRight" << std::endl;
            ...
            // 3. Send pick right request
            auto request = std::make_shared<ras_interfaces::srv::PickRight::Request>();
            request->target_pose = pose_msg.value();
            request->grip_state = grip_state;

            auto future = pick_right_client->async_send_request(request);
            if (rclcpp::spin_until_future_complete(node_, future) != 
                rclcpp::FutureReturnCode::SUCCESS)
            {
                RCLCPP_ERROR(node_->get_logger(), "Failed to send pick right request");
                return BT::NodeStatus::FAILURE;
            }

            auto response = future.get();
            if (!response->success) {
                RCLCPP_ERROR(node_->get_logger(), "Pick right operation failed: %s", response->message.c_str());
                return BT::NodeStatus::FAILURE;
            }

            RCLCPP_INFO(node_->get_logger(), "PickRight completed successfully");
            return BT::NodeStatus::SUCCESS;
        }

    private:
        rclcpp::Node::SharedPtr node_;
        rclcpp::Client<ras_interfaces::srv::PickRight>::SharedPtr pick_right_client;
    };
} 